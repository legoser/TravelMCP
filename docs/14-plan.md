...
- **D-1. Blocking в `matchStops` (пробел №1 ревью).** Сейчас: `cands`
  строится из ВСЕХ терминалов на каждый трип (`trips_attach.go`) — O(T×S×N):
  пилот 3.1М пар (28с, порог боли не перейдён — потому и не замечен), РФ
  ≈ 15K×4×120K = 7.2 млрд пар ≈ 18 ч/прогон. Точностная цена: стоп без
  координат конкурирует по имени со всей страной. **Решение:** по лекалу
  `JoinPager.prefilter` (НЕ голый `geo.SpatialIndex.Nearest` — тот выкинет
  код-совпадения, а golden-кейс «тёзка за 79 км верифицируется по коду»
  обязан проходить): пул = гео-окно 2км ∪ код-совпадения ∪ терминалы без
  координат; hoist `cands`/индекса из per-trip в per-run (`AttachTrips`
  строит один раз). Готово: тест blocking (код-матч за пределами окна
  проходит, далёкий без кода — нет) + замер времени на фикстуре.
- **D-2. Greedy exclusivity внутри рейса (пробел №2 ревью).** Сейчас: каждый
  стоп матчится независимо — два стопа рейса могут сойтись на одном
  терминале; `collapseConsecutive` лечит соседний симптом, не причину; при
  дефиците скелета конкуренция за терминалы растёт. **Решение:** `used` по
  terminalID внутри `matchStops`; занятый терминал недоступен следующим
  позициям того же рейса (кольцевые первый==последний — легитимны, §5.1).
  Готово: тест (два стопа, один очевидный терминал → первый берёт, второй
  уходит в unmatched, не дубль).
- **D-3. Backbone/provisional доводка (пробел №3 ревью).** All-or-nothing
  промоушен (любой unmatched → весь трип в staging) умножает единичную
  слабость на рейс — структурная причина 0/391. `is_provisional` (C-1) уже
  в DDL/модели; достроить ветку промоушена: трип с verified-концами и
  provisional-серединой промоутится (не staging), валидаторы гоняются по
  эффективной последовательности (после пропуска rejected-промежутка —
  перегон через пропуск должен пересчитаться, иначе ложноотрицательный
  overspeed). Готово: тест промоушена с provisional-серединой + тест
  валидаторов на «склеенном» перегоне.
- **D-4. `stop_times.match_score/match_method` (пробел №4 ревью).** Скор и
  метод матчинга стираются при промоушене → запланированный §8 FP-аудит
  нечем делать (выборка маргинальных матчей невозможна). **Решение:**
  аддитивно в `001_initial.sql` (`match_score real`, `match_method text`),
  `MatchedStopTime.Score` из `matchStops`, персист в `persistPromotedTrip`.
  Готово: миграция 0 ошибок + round-trip тест (score читается обратно).
- **D-5. Единая формула скоринга (дубль, вскрытый ревью).** `PairScore`
  (join.go) и `pairScoreCached` (join_stream.go) — две копии одной формулы,
  рассинхронизируются молча. **Решение:** вынести общее тело
  `scorePairBody(a, b, cfg, nameSimFn func(a,b string) float64)`; `PairScore`
  зовёт с прямым namesim, `pairScoreCached` — с мемоизированной обёрткой.
  `Join()` НЕ удалять — живой потребитель `legacy_match.go` (вход 1×N,
  blocking там бессмыслен). Готово: existing-тесты зелёные без правок
  ожиданий (формула не изменилась).
