...
- **Трек Г (частично done).** Done 2026-09-15 (шаг 1): движок
  ...
  данные, не код.
- **Квота (решение 2026-09-15).** Семантика: `api_quotas.used` — только
  успешные вызовы; `api_calls` — журнал попыток. `TryConsumeQuota`
  резервирует до handler'а (защита от превышения), неуспех возвращает
  резерв через `RefundQuota` (floor 0, три реализации: postgres/pgTx/
  memory; интерфейс стора не расширен — assertion в воркере).
