...
Баг закрыт: возврат квоты при неуспехе (`RefundQuota`, used=только успехи;
попутно пойман и закрыт полуночный флейк контракта квот — день берётся
из стора, а не из локальных часов).
Трек Г шаг 1: движок `mcraptor` (дефолт `csa` цел), паритет + тест выбора.
Шаг 2: нативный фронт Парето при `engine=mcraptor` (до 6 перепрогонов → 1
доп. поиск; эвристика осталась для csa/raptor).
Шаг 3: demand-срез (`trip_demand`, тики из `find_route`, stale-query,
контракт на обоих бэкендах); TTL-ресинк-воркер — следующая фаза.
