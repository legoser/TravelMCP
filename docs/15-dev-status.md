...
- **A-фиксы конвейера сбора (до пересбора БД, 2026-09-11):**
  (a) **tz-конверсия flatten** — времена Яндекс-ниток местные, канон хранит
  UTC (§3.13): сдвиг зоны извлекается из schedule-события
  (`ParseRaspTzShift` на `it.Departure` «…+07:00», per-нитку) →
  `RaspFlattenConfig.TzShiftMin` → `toUtcMinutes` в flatten
  ((local−shift+1440)%1440 до rollMidnight — переход через полночь
  остаётся корректным, UTC-полночь длинных рейсов не ломает
  монотонность). Баг: 06:30 местного хранилось как 06:30 «UTC» =
  13:30 местного — все утренние рейсы уезжали на 7 часов. Тесты:
  ParseRaspTzShift, TzShift / CrossUtcMidnight / LongTrip.
  (b) **Backbone-промоут для межгорода** — если оба конечных стопа
  нитки заматчены verified, трип промоутится с mid_gaps независимо от
  доли 2/3 (перегоны через пропуск валидируются на эффективных
  соседях): «Юрга—Кемерово» 2/16 стопов больше не skeleton_gap
  (`backbonePromotable` по StopID концов; тесты IntercityEndsPromote /
  IntercityOneEndStaged).
  (c) `RaspThread.FuzzyTimes` удалён (мёртвое поле; fuzzy-семантика
  живёт в FlatStop.IsFuzzy + interpolateFuzzyTimes §5.7).
