...
function jobPayloadDetail(r){try{const p=JSON.parse(r.Payload||'{}');const parts=[];if(p.kind)parts.push(p.kind);if(p.terminal_id)parts.push('term '+p.terminal_id);if(p.transport)parts.push(p.transport);if(p.date)parts.push(p.date);if(p.offline!==undefined)parts.push(p.offline?'кэш':'API');if(p.tag&&p.tag!==('collect-trips-term-'+p.terminal_id))parts.push(p.tag);return parts.join(' · ')}catch(e){return ''}}
async function loadJobs(){try{const j=await api('/api/v1/jobs');const tb=$('jobsBody');tb.innerHTML='';(j||[]).forEach(r=>{const tr=document.createElement('tr');const canReset=['dead','retry','running','pending','cancelled'].includes(r.State);const canCancel=['pending','retry','running'].includes(r.State);const det=jobPayloadDetail(r);const region=r.Region||((()=>{try{const p=JSON.parse(r.Payload||'{}');return p.terminal_id?('term '+p.terminal_id):''}catch(e){return ''}})())||'—';tr.innerHTML='<td>'+r.ID+'</td><td>'+r.Type+(det?'<br><span class="muted" style="font-size:11px">'+det+'</span>':'')+'</td><td>'+region+'</td><td><span class="badge '+r.State+'">'+r.State+'</span></td><td>'+r.Attempts+'</td><td>'+(r.NextRun||'—')+'</td><td style="max-width:200px;overflow:hidden;text-overflow:ellipsis" title="'+(r.LastError||'').replace(/"/g,'&quot;')+'">'+(r.LastError||'').slice(0,60)+'</td><td>'+(canCancel?'<button class="danger" onclick="cancelJob('+r.ID+')" title="Отменить: не стартовавший снимается из очереди; работающий конвейер остановится на ближайшей станции">✕</button> ':'')+(canReset?'<button class="secondary" onclick="resetJob('+r.ID+')" title="Перезапустить (attempts=0, pending)">↻</button>':'<span class="muted">—</span>')+'</td>';tb.appendChild(tr)});if((j||[]).length===0) tb.innerHTML='<tr><td colspan="8" class="muted">jobs нет</td></tr>'}catch(e){$('jobsBody').innerHTML='<tr><td colspan="8" class="muted">'+e.message+'</td></tr>'}}
async function cancelJob(id){if(!confirm('Отменить job '+id+'? Не стартовавший — снимается из очереди; работающий конвейер остановится на ближайшей станции (собранное в кэше и каноне сохранится).')) return;try{const j=await api('/api/v1/jobs/'+id+'/cancel',{method:'POST'});toast('Job '+j.id+' → cancelled');loadJobs()}catch(e){toast(e.message,true)}}
...
async function loadTermSchedule(id){
  ...
  try{
    ...
    (j.items).forEach(it=>{
      ...
      h+='<tr><td>'+it.departure+'</td><td><b>'+t+'</b></td><td style="max-width:280px">'+(it.destination||'—')+'</td><td>'+(it.mode||'')+'</td><td>'+it.external_trip_code+'</td><td class="muted">'+(it.service_days||'')+'</td><td><button class="secondary" title="Стопы рейса" onclick="showSchedTripStops('+it.trip_id+',this)">→</button></td></tr>';
    });
    h+='</tbody></table></div>';
    h+='<div id="schedTripStops" style="display:none" class="card" style="margin-top:8px"></div>';
    out.innerHTML=h;
  }catch(e){out.innerHTML='<span class="muted">Ошибка: '+e.message+'</span>'}
}
async function showSchedTripStops(tripID,btn){
  let box=$('schedTripStops_'+tripID);
  if(box){const open=!box.classList.contains('hidden');box.classList.toggle('hidden',open);return}
  box=document.createElement('div');box.id='schedTripStops_'+tripID;box.className='card';box.style.cssText='margin:6px 0;padding:10px;max-height:280px;overflow:auto';box.innerHTML='<span class="muted">загрузка стопов рейса '+tripID+'…</span>';
  btn.closest('tr').after(box);
  try{
    ...
    let h='<div class="row" style="justify-content:space-between"><b style="font-size:13px">Рейс '+tripID+' — стопы</b><button class="secondary" onclick="$(\'schedTripStops_'+tripID+'\').remove()">✕</button></div><table style="margin-top:6px"><thead><tr><th>#</th><th>приб.</th><th>отпр.</th><th>стоп</th><th></th></tr></thead><tbody>';
    (j.items||[]).forEach(st=>{h+='<tr><td>'+st.seq+'</td><td>'+fmt(st.arrival)+'</td><td>'+fmt(st.departure)+'</td><td style="max-width:320px">'+(st.name||'—')+'</td><td>'+(st.is_provisional?'<span class="badge pending" title="provisional">P</span>':'')+'</td></tr>'});
    if(!(j.items||[]).length)h+='<tr><td colspan="5" class="muted">стопов нет</td></tr>';
    h+='</tbody></table>';
    box.innerHTML=h;
  }catch(e){box.innerHTML='<span class="muted">Ошибка: '+e.message+'</span>'}
}
...
let colRegionsState={q:'',country:'',offset:0,limit:30,total:0,countries:[]};
async function loadCollectRegions(){const st=colRegionsState;if($('colRegionSearch'))st.q=$('colRegionSearch').value.trim();if($('colRegionCountry'))st.country=$('colRegionCountry').value;try{const j=await api('/api/v1/collect/regions?q='+encodeURIComponent(st.q)+'&country='+encodeURIComponent(st.country)+'&limit='+st.limit+'&offset='+st.offset);const sel=$('colRegion');const cur=sel.value;if(!(j.items||[]).length&&st.offset===0){sel.innerHTML='<option value="">— дампа нет —</option>';$('colRegionsBody').innerHTML='<tr><td colspan="4" class="muted">'+((st.q||st.country)?'ничего не найдено':'Яндекс-дамп не найден (sync.yandex_dump_path)')+'</td></tr>';st.total=j.total||0;renderRegionsPager();return}sel.innerHTML='';(j.items||[]).forEach(r=>{const o=document.createElement('option');o.value=r.region;o.textContent=r.region+' ('+r.terminal_stations+')';sel.appendChild(o)});if(cur)sel.value=cur;if(!sel.value&&(j.items||[]).length)sel.value=j.items[0].region;const tb=$('colRegionsBody');tb.innerHTML='';(j.items||[]).forEach(r=>{const tr=document.createElement('tr');const esc=r.region.replace(/'/g,"\\'");tr.innerHTML='<td>'+r.region+'</td><td class="muted">'+(r.country||'—')+'</td><td>'+r.terminal_stations+'</td><td><button class="secondary" onclick="pickRegion(\''+esc+'\')">выбрать</button></td>';tb.appendChild(tr)});if(!(j.items||[]).length)tb.innerHTML='<tr><td colspan="4" class="muted">нет регионов на странице</td></tr>';st.total=j.total||0;st.countries=j.countries||[];const cSel=$('colRegionCountry');if(cSel){const c=cSel.value;cSel.innerHTML='<option value="">все страны</option>'+st.countries.map(x=>'<option value="'+x.replace(/"/g,'&quot;')+'">'+x+'</option>').join('');cSel.value=c}renderRegionsPager()}catch(e){$('colRegionsBody').innerHTML='<tr><td colspan="4" class="muted">'+e.message+'</td></tr>'}}
function renderRegionsPager(){const st=colRegionsState;const p=$('colRegionsPager');if(!p)return;const from=st.total===0?0:st.offset+1,to=Math.min(st.offset+st.limit,st.total);p.innerHTML='<span class="muted" style="font-size:12px">'+from+'–'+to+' из '+st.total+'</span> <button class="secondary" '+(st.offset<=0?'disabled':'')+' onclick="colRegionsPage(-1)">←</button> <button class="secondary" '+(st.offset+st.limit>=st.total?'disabled':'')+' onclick="colRegionsPage(1)">→</button>'}
function colRegionsPage(dir){const st=colRegionsState;st.offset=Math.max(0,st.offset+dir*st.limit);loadCollectRegions()}
function colRegionsFilter(){colRegionsState.offset=0;loadCollectRegions()}
...
async function collectSkeleton(){const region=$('colRegion').value;if(!region) return toast('Выберите регион',true);const body={regions:[region],transports:colTransports(),station_types:colStationTypes(),offline:$('colOffline').checked};try{const j=await api('/api/v1/collect/skeleton',{method:'POST',body:JSON.stringify(body)});$('colMsg').textContent='Скелет: job #'+j.id+' поставлен в очередь (регион '+j.region+'). Статус — вкладка «Jobs», результат — «Импорты».';toast('Job #'+j.id+' создан')}catch(e){$('colMsg').textContent='Ошибка: '+e.message;toast(e.message,true)}}
async function collectTermTrips(id,btn){const out=$('termCollectOut');if(!out) return;const date=$('termCollectDate').value||new Date().toISOString().slice(0,10);const transport=$('termCollectTransport')?$('termCollectTransport').value:'bus';const offline=$('termCollectOffline')?$('termCollectOffline').checked:true;if(btn)btn.disabled=true;out.textContent='job ставится в очередь…';try{const j=await api('/api/v1/collect/trips',{method:'POST',body:JSON.stringify({terminal_id:id,date:date,transport:transport,offline:offline,tag:'collect-trips-term-'+id})});out.textContent='Job #'+j.id+' создан: сбор расписания терминала '+id+' ('+transport+', '+date+', '+(offline?'только кэш (все даты)':'кэш → API по квоте')+'). Статус — вкладка «Jobs»'+(offline?'':' (повтор при исчерпанной квоте — после её сброса)')+', результат — «Импорты».';toast('Job #'+j.id+' создан')}catch(e){out.textContent='Ошибка: '+e.message;toast(e.message,true)}finally{if(btn)btn.disabled=false}}
async function collectTrips(){const region=$('colRegion').value;if(!region) return toast('Выберите регион',true);const body={region:region,date:$('colDate').value||'',offline:$('colOffline').checked,stations:($('colStations')?$('colStations').value:'yandex'),station_types:colStationTypes()};try{const j=await api('/api/v1/collect/trips',{method:'POST',body:JSON.stringify(body)});$('colMsg').textContent='Рейсы: job #'+j.id+' поставлен в очередь (регион '+j.region+', станции: '+body.stations+'). Статус — «Jobs», результат — «Импорты». Cache-first (кэш за все даты); при offline — только сохранённые расписания/нитки, пустой offline-сбор завершается сразу без повторов.';toast('Job #'+j.id+' создан')}catch(e){$('colMsg').textContent='Ошибка: '+e.message;toast(e.message,true)}}
function colStationsChanged(){const v=$('colStations')?$('colStations').value:'yandex';if(v==='overpass'&&$('colOffline').checked){toast('Overpass-сбор не работает offline: снимите «только кэш» или выберите Яндекс-дамп',true)}}
async function collectRoutesOSM(){const region=$('colRegion').value;if(!region) return toast('Выберите регион',true);const body={region:region,offline:false,force:false};try{const j=await api('/api/v1/collect/routes',{method:'POST',body:JSON.stringify(body)});$('colMsg').textContent='Overpass: job #'+j.id+' поставлен в очередь (регион '+j.region+'). Терминалы и маршруты из OSM без расписания; времена добираются Яндексом позже (staging awaiting_times).';toast('Job #'+j.id+' создан')}catch(e){$('colMsg').textContent='Ошибка: '+e.message;toast(e.message,true)}}
